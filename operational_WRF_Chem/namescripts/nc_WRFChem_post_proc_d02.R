...
# gerate a time sequence for the WRF-Chem run at intervals of 1 hour (should be 73 images), 3 days
...
qq <- 0
   ...
   # writeRaster(r, paste0(DateTime,".tif") , options= "INTERLEAVE=BAND", overwrite=T)
    writeRaster(r, paste0(current_dir, "/PM10/",DateTime,".tif") , options= "INTERLEAVE=BAND", overwrite=T)
   }
